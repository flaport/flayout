# AUTOGENERATED! DO NOT EDIT! File to edit: source/03_cell.ipynb (unless otherwise specified).

__all__ = ['reference', 'copy_tree', 'add_pcells_to_layout', 'add_lib_pcell_to_layout', 'add_lib_cell_to_layout',
           'add_cell_to_layout']
...
# Internal Cell
SUPPORTED = [pya.Cell, GdsPyCell]
SUPPORTED = [Cell.__name__ for Cell in SUPPORTED if Cell is not DoesNotExist]
...
def copy_tree(source: pya.Cell, dest: pya.Cell, on_same_name: str = "skip"):
    ...
    if isinstance(source, pya.Cell):
        _copy_klayout(source, dest, on_same_name)
    elif isinstance(source, GdsPyCell):
        _copy_gdspy(source, dest, on_same_name)
    else:
        raise ValueError(
            f"Error in copy_tree: source is not a supported cell type. "
            f"Got: {type(source)}. Expected: {', '.join(SUPPORTED)}."
        )
    return dest
...
def _validate_on_same_name(on_same_name):
    ...
    if not on_same_name in allowed_on_same_name:
        raise ValueError(
            "one_same_name should be one of the following: "
...
def _copy_klayout(source, dest, on_same_name, depth=0):
    ...
    for idx in source.each_child_cell():
        ...
        source_cells_map[idx] = add_cell_to_layout(
...
def add_cells_to_layout(
    ...
    for cell in cells:
        add_cell_to_layout(layout, cell, on_same_name, depth)
...
# Cell
def add_lib_cell_to_layout(layout: pya.Layout, lib: pya.Library, cell: pya.Cell):
    ...
    if pcell is not None:
        return add_lib_pcell_to_layout(layout, lib, pcell, cell.pcell_parameters())
    else:
        cell_idx = lib.layout().cell_by_name(cell.name)
        return layout.add_lib_cell(lib, cell_idx)

# export
def add_cell_to_layout(
    ...
    """
    if cell.is_proxy():
        add_lib_cell_to_layout(layout, cell.library(), cell)
